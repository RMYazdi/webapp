...
function fileChange2(e) {
    ...
    if (file.type == "image/jpeg" || file.type == "image/png") {
        ...
        // Activate Submit Button
        el('submit_btn').type = "submit";
        document.getElementById('inp_img2').value = '';
        var reader = new FileReader();  
        reader.onload = function(readerEvent) {
            ...
            image.onload = function(imageEvent) {	
                ...
                el('image-picked2').src = dataURL;
                el('image-picked2').className = '';
                // before sending to server, split dataURL to send only data bytes
                data_bytes = dataURL.split(',');
                document.getElementById('inp_img2').value = data_bytes[1];
                // save local data_bytes[1]
                localStorage.setItem("imgData", data_bytes[1]);
                var dataImage = localStorage.getItem('imgData');
                // console.log("data_bytes[1]:", data_bytes[1]);
        ...
        document.getElementById('inp_file2').value = '';	
        alert('Please select image only in JPG or PNG format!');	
    }
}
...
function fileChange3(e) {
    ...
    if (file.type == "image/jpeg" || file.type == "image/png") {
        ...
        // Activate Submit Button
        el('submit_btn').type = "submit";
        document.getElementById('inp_img3').value = '';
        var reader = new FileReader();  
        reader.onload = function(readerEvent) {
            ...
            image.onload = function(imageEvent) {	
                ...
                el('image-picked3').src = dataURL;
                el('image-picked3').className = '';
                // before sending to server, split dataURL to send only data bytes
                data_bytes = dataURL.split(',');
                document.getElementById('inp_img3').value = data_bytes[1];
                // save local data_bytes[1]
                localStorage.setItem("imgData", data_bytes[1]);
                var dataImage = localStorage.getItem('imgData');
                // console.log("data_bytes[1]:", data_bytes[1]);
        ...
        document.getElementById('inp_file3').value = '';	
        alert('Please select image only in JPG or PNG format!');	
    }
}
